...
def api_move_calendar_day(project_id):
    ...
    try:
        ...
        # Recalculate sun times for all days (handles moved days with new dates)
...
def api_create_project_version(project_id):
    ...
    try:
        ...
        existing_versions = get_project_versions(project_id, user_id)
        if any(v['versionNumber'] == version_number for v in existing_versions):
            return jsonify({'error': 'Version number already exists'}), 400
            
        new_version = create_project_version(project_id, version_number, notes, user_id)
...
def api_calculate_sun_times():
    ...
    try:
        from utils.sun_utils import get_sun_times_for_location
